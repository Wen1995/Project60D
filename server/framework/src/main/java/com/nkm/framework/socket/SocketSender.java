...
import com.nkm.framework.console.disruptor.MyDisruptor;
import com.nkm.framework.console.disruptor.ObjectEvent;
            ...
            if (tPackets != null && tPackets.length > 0) {
                ...
                if (receivers != null && receivers.size() > 0) {
                    for (Long uid : receivers) {
                        Channel channel = GameServer.GetInstance().getChannel(uid);
                ...
                // 对单的推送
                    Channel channel = GameServer.GetInstance().getChannel(p.getUid());
                    if (channel == null || !channel.isOpen() || !channel.isActive()) {
                        logger.info("SocketSender send fail TPacketId:" + p.getCmd() + " Uid:"
                                + p.getUid());
                        return;
